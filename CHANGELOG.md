...
- **Makefile / CI helpers**: `test-go`, `test-coverage`, `test-coverage-check`,
`coverage-inventory` (`scripts/coverage-inventory.sh`), and optional
`test-coverage-integration` (merged `-coverpkg` without `-short`). Default
`COVERAGE_PKGS` excludes `phaudittrailv0` from the merged profile; override
with `COVERAGE_PKGS=./...` when needed. Default gate remains
`COVERAGE_MIN=50` with `COVERAGE_GOAL=90`.
- **AMQP testability**: package-level `amqpDialHook` and same-package test hooks
on `AmqpClient` (`publishForTest`, `consumeForTest`, `channelCloseForTest`,
`connCloseForTest`) so `-short` tests can cover dial errors, publish/confirm,
consume, and close branches without a real broker.
- **Redis in tests**: `miniredis` + `resetRedisClientStateForTesting` for Echo
middleware (`VerifCsrf`, `VerifIdemKey`) integration-style table coverage.
- **wirepb gRPC**: bufconn tests for `Download` / presigned / view URL RPCs,
client-streaming `Upload`, unary server/client interceptors, and unimplemented
upload error path.
- **phaudittrailv0**: `TestPushMessage_nilQueue` for the nil-queue early exit.
- **Tests**: expanded merged coverage (~50% → ~59% statements with default
`COVERAGE_PKGS`) with root `logger`/`sentry`/`health`/`findEnvPath` exercises,
`SentryLoggingFromEnv`, wire protobuf + gRPC `Health` bufconn round-trip, and
`ReceiveLog` capture path in `phsentry`.
- **Coverage follow-up**: further AMQP accessor/matrix tests (`qosForTest`),
`RevokeToken` / `parsePublicKey` tests, miniredis CSRF/idempotency redis-error
