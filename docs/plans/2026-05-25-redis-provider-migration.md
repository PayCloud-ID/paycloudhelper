...
**Scope is lighter than Group A/B.** All five already delegate to the **pch shared pool** (`pchelper.GetRedisPoolClient()` via a thin `RedisHelper`; the only `redis.NewClient` calls are in `*_test.go`). So there is **no "delegate the pool" work** here — they need only the mechanical **v8→v9 import swap** (`github.com/go-redis/redis/v8` → `github.com/redis/go-redis/v9`; rename `IdleTimeout → ConnMaxIdleTime` if set) when they bump `paycloudhelper` to ~~v2.x~~ **v1.10.0 or later** *(corrected 2026-07-31: the go-redis v9 alignment shipped in `v1.10.0`, and **no `v2.x` is planned** — the library stays on the `v1.x` line for the foreseeable future. Read every "v2.x" in this dated plan as "`v1.10.0`+".)* None use redsync/distributed locks. **No key changes** — key naming is handled separately by the namespacing/TTL plan.
