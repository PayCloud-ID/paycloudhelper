...
**Status 2026-08-10: ✅ implemented locally on `feat/PA-222` for PA-225.** `paycloudhelper` is pinned
to `v1.11.1`, go-redis v9 is a direct dependency at `v9.18.0`, and all three manager import sites
use the v9 module path. Compatibility edits required by the v9 API (`ConnMaxLifetime` and explicit
contexts) are included. The implementation is intentionally left uncommitted and unpushed.

Track B remains independent and is not included because `pcauth`/`paycloudhelper v1.12.0` is not
available yet.
